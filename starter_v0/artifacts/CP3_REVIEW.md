...
| [Web 19:45](../transcripts/v0_openrouter_web_20260915T194527764448.transcript.json) | T1 tool trả degraded, assistant nói bình thường và 42ms. **Cập nhật 16/09:** không phải model diễn giải sai — đây là bug ở `ui.py` (`mock_deterministic_fallback`, đã sửa), tự chèn câu trả lời dựng sẵn "hoạt động bình thường, 42ms" khi provider lỗi (429), bất kể tool trả gì. Đã tái hiện trực tiếp: gọi `/api/chat` lúc provider đang rate-limit (429 thật) cho cùng câu hỏi, response có `fallback: true` và tool result thật vẫn là `degraded`/`INC-1042`. Transcript T1 này không phản ánh hành vi model, chỉ phản ánh bug UI đã sửa. |
| [Web 19:58:21](../transcripts/v0_openrouter_web_20260915T195821934813.transcript.json) | T1 cùng nguyên nhân: bug fallback ở `ui.py` nêu trên, không phải model diễn giải sai. |
...
**Bug UI phát hiện và đã sửa (16/09):** `starter_v0/ui.py` có `mock_deterministic_fallback` chạy khi lệnh gọi provider thật ném lỗi (vd. 429). Bản cũ trả về câu trả lời dựng sẵn theo từ khóa (vd. luôn nói VPN "hoạt động bình thường, 42ms") mà không đọc tool result thật, nên khi tool thật trả `degraded` (transcript 19:45 và 19:58:21 ở trên), UI vẫn hiển thị và LƯU vào transcript một câu khẳng định sai sự thật, không có nhãn cho biết đây là fallback chứ không phải model. Đây là vi phạm trực tiếp yêu cầu CP4 "UI phải hiển thị hành vi thật". Đã sửa: fallback giờ luôn gắn `fallback: true` + `provider_error` thật, và assistant_text chỉ echo nguyên văn JSON tool result thật (không tự diễn giải/khẳng định), UI có banner cảnh báo "FALLBACK" màu đỏ. Đã kiểm chứng trực tiếp bằng cách gọi `/api/chat` khi OpenRouter đang thật sự rate-limit (429) — response trả `fallback: true` và tool result thật (`status: degraded`, `incident_id: INC-1042`) thay vì câu khẳng định sai như trước.
